...
function dir_tool.getPathFileName(strurl, strchar, bafter)
  ...
  if (bafter == true) then
    result = string.sub(strurl, param1 + 1, string.len(strurl))
  else
    result = string.sub(strurl,1,param1)
  end

  -- 处理一下尾缀
  result = string.sub(result,1,string.len(result) - 4)
...
function dir_tool.get_dir_path(path_name, path)
  ...
  -- 截取下字符串从路径中提出名字
  local file_name = dir_tool.getPathFileName(path, tag, true)

  -- 替换符号
  local load_name = string.gsub(file_name,"\\",".")
